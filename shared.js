...
const MODELS = {
    ...
    'minimax-m2.5': { name:'MiniMax-M2.5', params:229e9, activeParams:10.5e9, layers:62, heads:48, kvHeads:8, hiddenDim:3072, headDim:128, interDim:1536, vocabSize:200064, maxCtx:196608, moe:true, numExperts:256, topK:8, arch:'MiniMax', softmaxAttnLayers:0, note:'Lightning Attention + MoE + MTP' },
    // Gemma
...
function populateModelSelect(selectEl) {
    ...
    for (const [k, m] of Object.entries(MODELS)) {
        ...
        else if (m.arch === 'MiniMax') g = 'MiniMax';
    ...
    selectEl.innerHTML = '';
    for (const g of ['LLaMA Family', 'Qwen Family', 'Mistral / Mixtral', 'DeepSeek', 'MiniMax', 'Phi', 'Gemma']) {
...
const I18N = {
    en: {
        ...
        'cfg.vendor': 'Vendor',
        'cfg.device': 'Device',
        'cfg.params': 'Runtime Configuration',
        ...
        'cfg.flops.util': 'Device FLOPS Utilization (%)',
        'cfg.mem.util': 'Device Memory Utilization (%)',
        'cfg.net.util': 'Device Network BW Utilization (%)',
    ...
    zh: {
        ...
        'cfg.batch': '批大小',
        'cfg.tp': '张量并行（GPU 数）',
        'cfg.flops.util': '设备 FLOPS 利用率（%）',
        'cfg.mem.util': '设备显存带宽利用率（%）',
        'cfg.net.util': '设备网络带宽利用率（%）',
        'cfg.flash': 'FlashAttention',
